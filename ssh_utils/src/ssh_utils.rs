...
mod tests {
    ...
    async fn create_test_group() -> Result<(SessionMap,SFTPMap),AnyError> { 
        let test_groups = vec![String::from("root@prod-agent1.recsa.prod"), String::from("root@prod-agent2.recsa.prod")];
    ...
    async fn dir_retrieval() {
        ...
        assert!(fs::exists(format!("{}/root@prod-agent1.recsa.prod", test_dir)).expect("Path should exist.")); 
        assert!(fs::exists(format!("{}/root@prod-agent2.recsa.prod", test_dir)).expect("Path should exist.")); 
    }
